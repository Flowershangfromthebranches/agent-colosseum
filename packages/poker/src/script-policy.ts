...
export function scriptDecide(kind: ScriptKind, legal: LegalAction[], handNo: number): AgentDecision {
  if (kind === 'check-fold') {
    if (legal.some((item) => item.action === 'check')) {
      return { action: 'check', publicRationale: 'script: check' }
    }
  ...
  if (kind === 'call-station') {
    ...
    if (legal.some((item) => item.action === 'call')) {
      return { action: 'call', publicRationale: 'script: call' }
    }
    return { action: 'fold', publicRationale: 'script: fold' }
  }
